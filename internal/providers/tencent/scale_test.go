...
func TestScaleBWPMetric(t *testing.T) {
    if scaleBWPMetric("InTraffic", 1) != 1000000 { t.Fatalf("bwp in bps") }
    if scaleBWPMetric("OutTraffic", 0.5) != 500000 { t.Fatalf("bwp out bps") }
    if scaleBWPMetric("InPkg", 10) != 10 { t.Fatalf("no scale pps") }
}
