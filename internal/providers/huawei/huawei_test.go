...
func TestHuaweiErrorClassifier(t *testing.T) {
	...
	tests := []struct {
		...
		{"APIGW.0308 error", &mockError{"APIGW.0308 rate limit exceeded"}, providerscommon.ErrorStatusLimit},
		{"ratelimit error", &mockError{"ratelimit exceeded"}, providerscommon.ErrorStatusLimit},
		{"rate limit error", &mockError{"rate limit exceeded"}, providerscommon.ErrorStatusLimit},
