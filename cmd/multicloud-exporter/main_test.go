...
func TestAdminAuthEnabled(t *testing.T) {
    ...
    if ok, _ := adminAuthEnabled(cfg); ok { t.Fatalf("disabled") }
    cfg.Server.AdminAuthEnabled = true
    cfg.Server.AdminAuth = []config.BasicAuth{{Username: "u", Password: "p"}}
    if ok, pairs := adminAuthEnabled(cfg); !ok || len(pairs) != 1 { t.Fatalf("enabled") }
}

func TestCheckBasicAuth(t *testing.T) {
    pairs := []config.BasicAuth{{Username: "a", Password: "b"}}
    if !checkBasicAuth(pairs, "a", "b") { t.Fatalf("good") }
    if checkBasicAuth(pairs, "a", "x") { t.Fatalf("bad") }
}
